...
| 2026-09-11 | Weekends: one chunk by default, more on request, after a pushback. Weekdays: 45-minute ceiling holds, and an extra chunk needs a stated reason | The original rule only said "weekday", leaving weekends undefined. On a weekend, cask-lead first spells out the next chunk's difficulty, workload and realistic time, and asks whether it can be finished (green and committed) today. If the owner still wants it, it is handed over. Friction rather than refusal keeps every extra chunk a deliberate choice. On weekdays the default answer to an extra chunk is no; an accepted exception is logged with its reason, so the pacer can see how often it happens. Agents never offer extra chunks |
| 2026-09-13 | In `internal/warmup` tests, the good-path `MoveOffset` calls discard their returned error on purpose. From Phase 1 onward, an error returned into a test is asserted | Owner's call, made knowingly at the end of chunk 0.4a: converting the two bare calls (`warmup_test.go:17` and `:23`) into nil-checks was proposed and declined. Go allows a call used as a statement to drop its results and `go vet` does not flag it, so this is a chosen gap, not an oversight. Scope is deliberately narrow — `internal/warmup` is scratch and is deleted when Phase 0 ends, so the habit does not travel. The decision is **not** reopened for the remaining warm-up chunks; it is simply superseded by the Phase 1 convention |
